...
class DeployScript:
    ...
    def __init__(self, environment: str = "production", dry_run: bool = False, skip_tests: bool = False):
    ...
    def check_prerequisites(self) -> None:
        ...
        if returncode != 0:
            ...
            self.log("Wrangler not authenticated, but CLOUDFLARE_API_TOKEN is available")
        else:
    ...
    def check_cloudflare_secrets(self) -> None:
        ...
        if returncode != 0:
            self.log(f"Could not list secrets for {self.environment}: {stderr}", "WARNING")
            self.log("Secrets may not be configured. Deployment will proceed without secret validation.", "INFO")
            return
    ...
    def deploy_server(self) -> None:
        ...
        self.run_command(
            ["wrangler", "deploy", "--env", self.environment],
            cwd=self.server_dir
        )
    ...
    def deploy_client(self) -> None:
        ...
        self.run_command(
            ["wrangler", "pages", "deploy", "dist", "--env", self.environment],
            cwd=self.client_dir
        )
    ...
    def save_deployment_log(self) -> None:
        ...
        log_file = self.project_root / f"deployment-{self.environment}-{datetime.now().strftime('%Y%m%d-%H%M%S')}.log"
